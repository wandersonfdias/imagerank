...
int read_training_set(char* training) {
	...
	*/
	...
	if (hasCache==0) {
		...
		FILE* fp = fopen("/tmp/cache/VAR_AUX.txt", "r");
			...
			*/
			if (N_ITEMSETS>0 && N_TRANSACTIONS>0) {
				...
				*/
				...
				// COUNT_TARGET
				fp = fopen("/tmp/cache/COUNT_TARGET.bin", "r");
				fread(&COUNT_TARGET, SIZEOF_COUNT_TARGET, 1, fp);
			    fclose(fp);

				// TARGET_ID
				fp = fopen("/tmp/cache/TARGET_ID.bin", "r");
				fread(&TARGET_ID, SIZEOF_TARGET_ID, 1, fp);
			    fclose(fp);

				// ITEMSETS
				fp = fopen("/tmp/cache/ITEMSETS.txt", "r");
				...
				// CLASS_NAME
				fp = fopen("/tmp/cache/CLASS_NAME.txt", "r");
				while (!feof(fp))
				{
					char key[1024];
					int value;
				...
				// SYMBOL_TABLE
				fp = fopen("/tmp/cache/SYMBOL_TABLE.txt", "r");
			    while (!feof(fp))
				{
					int key;
					char value[1024];
				...
				// ITEM_MAP
				fp = fopen("/tmp/cache/ITEM_MAP.txt", "r");
				while (!feof(fp))
				{
					char key[1024];
					int value;
				...
				// TID_CLASSES
				fp = fopen("/tmp/cache/TID_CLASSES.txt", "r");
			    while (!feof(fp))
				{
					int key, value;
	...
	if (hasCache==0) {
		...
		*/
		...
		// VARIAVEIS AUXILIARES
		FILE* fp = fopen("/tmp/cache/VAR_AUX.txt", "w");
		fprintf( fp
			   , "N_TRANSACTIONS=%d N_ITEMSETS=%d SIZEOF_CLASS_NAME=%lu SIZEOF_COUNT_TARGET=%lu SIZEOF_TARGET_ID=%lu SIZEOF_ITEMSETS=%lu SIZEOF_SYMBOL_TABLE=%lu SIZEOF_ITEM_MAP=%lu SIZEOF_TID_CLASSES=%lu"
			   , N_TRANSACTIONS
			   , N_ITEMSETS
		...
		// COUNT_TARGET
		fp = fopen("/tmp/cache/COUNT_TARGET.bin", "w");
		fwrite(&COUNT_TARGET, sizeof(COUNT_TARGET), 1, fp);
	    fclose(fp);

		// TARGET_ID
		fp = fopen("/tmp/cache/TARGET_ID.bin", "w");
		fwrite(&TARGET_ID, sizeof(TARGET_ID), 1, fp);
	    fclose(fp);

	    // ITEMSETS
		fp = fopen("/tmp/cache/ITEMSETS.txt", "w");
		for (int i=0; i<N_ITEMSETS; i++) {
		...
		// TID_CLASSES
		fp = fopen("/tmp/cache/TID_CLASSES.txt", "w");
		...
		// CLASS_NAME
		fp = fopen("/tmp/cache/CLASS_NAME.txt", "w");
		...
		// SYMBOL_TABLE
		fp = fopen("/tmp/cache/SYMBOL_TABLE.txt", "w");
		...
		// ITEM_MAP
		fp = fopen("/tmp/cache/ITEM_MAP.txt", "w");
