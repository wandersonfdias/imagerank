package br.ufmg.dcc.imagerank.shell.util;
...
public class ShellCommandRunner
{
	...
	private void createOutputDir() throws ProcessorException
	{
		...
		if (dir.exists())
		{
			...
			{
				throw new ProcessorException(String.format("Ocorreu um erro ao excluir o diretório de saída '%s'.", this.diretorioSaidaProcessamento), e);
			}
		}
		...
		if (!dirCreated)
		{
			throw new ProcessorException(String.format("Não foi possível criar o diretório de saída '%s'.", this.diretorioSaidaProcessamento));
		}
	}
